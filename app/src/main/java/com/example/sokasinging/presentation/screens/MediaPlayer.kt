...
@Composable
    fun ExoMediaPlayer(songTitle: String, songUrl: String) {
        ...
        // Create player with proper error handling, guarded for preview
        val exoPlayer = remember {
            if (isPreview) {
                null // ExoPlayer cannot be created in Preview
            } else {
        ...
        LaunchedEffect(songUrl) {
            exoPlayer?.let {
                try {
                    ...
                    it.setMediaItem(MediaItem.fromUri(songUrl))
                    it.prepare()
